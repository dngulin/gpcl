...
impl QmlLauncher {
    fn get_item_field(&self, index: usize, role: i32) -> Option<QVariant> {
        ...
        let field_id = ItemField::from_repr(role)?;

        let value = match field_id {
            ItemField::Name => QString::from(item.name.as_str()).to_qvariant(),
            ItemField::Icon => QString::from(item.icon.as_str()).to_qvariant(),
        };
...
impl QAbstractListModel for QmlLauncher {
    ...
    fn role_names(&self) -> HashMap<i32, QByteArray> {
        let map = ItemField::iter().map(|field_id| {
